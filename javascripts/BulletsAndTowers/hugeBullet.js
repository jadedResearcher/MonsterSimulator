...
function HugeBullet(x,y, target){
	this.x = x;
	this.y = y;
	this.imageString = "cannonBullet.png";
	this.maxSpeed = 5;
	this.magnitude = 0;  
	this.direction = Math.floor(Math.random()*360)* (Math.PI / 180);
	this.width = 0;
	this.height = 0;
	this.lifespan = 100; //bullets don't travel forever.
	this.target = target;
	...
	this.hit = function(monsters, monster, bullets){
		...
		//removeFromArray(bullets, this); //nothing stops a cannon ball
	}
