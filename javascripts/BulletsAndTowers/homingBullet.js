//chases after target
function HomingBullet(x,y, target){
	this.x = x;
	this.y = y;
	this.imageString = "homingBullet.png";
	this.maxSpeed = 7;
	this.magnitude = 0;  
	this.direction = Math.floor(Math.random()*360)* (Math.PI / 180);
	this.width = 0;
	this.height = 0;
	this.lifespan = 200; //bullets don't travel forever.
	this.target = target;
	...
	this.tick = function(canvas, monsters, bullets){
		if(this.lifespan < 0 || monsters.indexOf(this.target) == -1){
			//console.log("dying from lack of  or because target is dead")
			removeFromArray(bullets, this);
		}
		this.lifespan += -1;
		this.aim(); //always aim cause homing.
		this.move(canvas);
